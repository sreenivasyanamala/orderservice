...
import com.eatza.order.client.DeliveryServiceClient;
...
public class OrderServiceImpl implements OrderService {
	...
	@Autowired
	KafkaTemplate<String, DeliveryRequestDto> kafkaTemplate;
	...
	public Order placeOrder(OrderRequestDto orderRequest) throws OrderException {
		...
		for(OrderedItemsDto itemDto: itemsDtoList) {
			...
			try {
			  ...
			  deliveryDto.setCustomerId(orderRequest.getCustomerId());
			  deliveryDto.setRestaurantId(orderRequest.getRestaurantId());
			  //deliveryServiceClient.getdelivery(deliveryDto);
			 // HttpEntity<DeliveryRequestDto> entity = new HttpEntity<>(deliveryDto); String
			 // item = restTemplate.postForObject(deliveryServiceUrl, entity, String.class);
			
			kafkaTemplate.send("kafka_delivery_json", deliveryDto);
		}
